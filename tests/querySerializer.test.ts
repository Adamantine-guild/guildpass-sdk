...
import {
  ...
  serializeQuery,
} from "../src/utils/querySerializer.js";

describe("serializeQuery", () => {
  it("serializes scalar values with deterministic key ordering", () => {
    expect(
      serializeQuery({ z: true, count: 12.5, active: false, empty: "" })
    ).toBe("active=false&count=12.5&empty=&z=true");
  });

  it("omits null and undefined values", () => {
    expect(serializeQuery({ after: undefined, before: null, page: 2 })).toBe(
      "page=2"
    );
  });
  ...
  it("serializes Dates as UTC ISO-8601 strings", () => {
    expect(
      serializeQuery({ at: new Date("2026-08-26T12:34:56.789-05:00") })
    ).toBe("at=2026-08-26T17%3A34%3A56.789Z");
  });
  ...
  it("encodes reserved characters once", () => {
    ...
    expect(new URLSearchParams(serialized).get("redirect")).toBe(
      "a/b?x=1&y=two words"
    );
  });
  ...
  it("rejects non-finite numbers and invalid Dates", () => {
    ...
    expect(() => serializeQuery({ at: new Date("invalid") })).toThrow(
      InvalidQueryValueError
    );
  });
  ...
  it("rejects unsupported array items at runtime", () => {
    const invalid = { values: ["ok", true] } as unknown as Parameters<
      typeof serializeQuery
    >[0];
